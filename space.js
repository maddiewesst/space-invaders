...
function updateLaser(){
  ...
  for(i = 0; i < lasers.length; i++){
    ...
    for(j = 0; j < enemies.length; j++){
      ...
      if(collision(enemyRect, laserRect)){
        playSound("bullet")
        deleteLaser(lasers, laser, laser.$laser)
...
function updateEnemies() {
    ...
    for (i = 0; i < enemies.length; i++){
     ...
     // setting enemy cooldown to random and calling create enemy laser function if cooldown is 0
...
function updateEnemyLaser($container){
  ...
  for(let i = 0; i < enemyLasers.length; i++){
    ...
    if(collision(spaceshipRect, enemyLaserRect)){
      playSound("shot2");
      deleteLaser(enemyLasers, enemyLaser, enemyLaser.$enemyLaser);

      // update lives after collision
      state.lives--
